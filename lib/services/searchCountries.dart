            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  if (indexingAll) {
                    index = sortedIndexKeysTotalConfirmed[
                        (sortedIndexKeysTotalConfirmed.length - 1) - index];
                  } else {
                    index = index;
                  }
                  ...
                  return GestureDetector(
                    onTap: () {
                      ...
                      if (indexingAll) {
                        ...
                        int indexNew = summaryData.countries
                            .indexOf(filteredDataList[index]);
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => StatisticsScreen(
                      ...
                      children: <Widget>[
                        Container(
                          margin: EdgeInsetsDirectional.fromSTEB(
                              UiElements.padding * 1.5,
                              0,
                              UiElements.padding * 1.5,
                              UiElements.padding),
                          width:
                              UiElements.screenWidth - UiElements.padding * 3,
                          height: UiElements.screenHeight * .06,
                          decoration: BoxDecoration(
                            color: UiElements.cardViewColor3,
                            borderRadius:
                                BorderRadius.circular(UiElements.padding),
                          ),
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: <Widget>[
                              Container(
                                width: (UiElements.screenWidth -
                                        3 * UiElements.padding) -
                                    UiElements.screenHeight * .2,
                                alignment: Alignment.centerLeft,
                                height: UiElements.screenHeight * .13,
                                padding:
                                    EdgeInsets.only(left: UiElements.padding),
                                decoration: BoxDecoration(
                                  //color: Colors.white,
                                ...
                                child: Text(
                                  filteredDataList[index].country,
                              ...
                              Container(
                                  padding: EdgeInsets.only(
                                      right: UiElements.padding),
                                  //width: UiElements.screenHeight*.2,
                                  height: UiElements.screenHeight * .13,
                                  child: Flags.getMiniFlag(
                                      '${filteredDataList[index].countryCode}',
                                      20,
                                      20)),
                            ],
                          ),
...
Void findSortedInex() {
  if (findingSortedIndexFirst) {
    ...
    sortedIndexKeysTotalConfirmed =
        sortedIndexKeysTotalConfirmedMap.keys.toList();
    sortedIndexKeysTotalRecovered =
        sortedIndexKeysTotalRecoveredMap.keys.toList();
