...
LineChartData plotData() {
  ...
  return LineChartData(
    ...
    maxY: (numberofCasesConfirmedListTotal.isEmpty)
        ? 0
        : numberofCasesConfirmedListTotal.reduce(max),
    minY: (numberofCasesConfirmedListTotal.isEmpty)
        ? 0
        : numberofCasesConfirmedListTotal.reduce(min),
...
List<LineChartBarData> linesBarData1() {
  LineChartBarData lineChartBarConfirmed = LineChartBarData(
    ...
    isCurved: true,
    colors: [Color(0xff4af699), Color(0xff4af699)],
    curveSmoothness: 0,
    barWidth: 4,
  ...
  LineChartBarData lineChartBarRecovered = LineChartBarData(
    ...
    isCurved: true,
    colors: [Color(0xffaa4cfc), Color(0xffaa4cfc)],
    curveSmoothness: 0,
    barWidth: 4,
    isStrokeCapRound: true,
    dotData: FlDotData(
  ...
  LineChartBarData lineChartBarDeaths = LineChartBarData(
    ...
    isCurved: true,
    colors: [Color(0xff27b6fc), Color(0xff27b6fc)],
    curveSmoothness: 0,
    barWidth: 4,
    isStrokeCapRound: true,
    dotData: FlDotData(
    ...
    for (int i = 0; i < caseDateRecoveredListTotal.length; i++) {
      for (int j = 0; j < mnth32.length; j++) {
        if (caseDateRecoveredListTotal[i].month == mnth32[j]) {
          xData = caseDateRecoveredListTotal[i].month +
              1 / divimnth32date[j] * caseDateRecoveredListTotal[i].day;
          xDateData.add(xData);
      ...
      if (isMnth32 == false) {
        xData = caseDateRecoveredListTotal[i].month +
            1 / 30 * caseDateRecoveredListTotal[i].day;
        xDateData.add(xData);
      }
    }
  } catch (e) {
